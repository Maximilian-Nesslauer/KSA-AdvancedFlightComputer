...
**Burning solid motors in the staging readout.** The game's staging model paces a burning solid motor at the grain left over the full grain's burn time, so a booster keeps its whole burn time on every refresh and a nearly spent one is shown as a trickle that outlasts the core. The mod paces it at the mass flow the motor reports instead, so the in-flight staging window and the guidance stage list end the booster phase where the boosters do. Early in a progressive burn the remaining time is still an estimate from the current flow and settles as the burn goes on.
