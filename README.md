...
- Thruster firing uses an **LP allocator**: the classic fuel-optimal jet-select linear program over the raw per-thruster wrenches with zero net torque folded into the constraints (the Bergmann/Draper formulation, flown on ATV and Orion). It exploits off-axis thrusters the stock grouping ignores and produces torque-free firing patterns. Layouts where the zero-torque LP is infeasible fall back automatically to a stock-consistent axis-group allocator that leaves residual torque to the attitude hold.
