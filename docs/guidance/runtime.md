...
| `Ui` | The panel shell in `Panel.cs`, the per-tab content in `Gauges`, the world-space overlays in `Overlays`, and the per-frame entry point that draws them in `Window.cs`. |
