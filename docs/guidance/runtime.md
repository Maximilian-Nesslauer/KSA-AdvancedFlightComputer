...
- The staging block, `SolidPacingPatch.Apply`, installs a postfix on the private `SequencePerformanceList.ComputeSolidPacingMassFlowRate` that paces a burning solid motor at the mass flow its nozzles report. The game paces it at the grain left over the full grain's burn time, so its remaining burn never shortens and a nearly spent booster is modelled as a trickle that outlasts the core. The patch is its own feature block, because the game's staging window reads the same model and stays right whether or not guidance flies.
