...
nonconvexity - and the source of several separate pathologies (section 7).
...
> 3 km with a scale sized for 300 m and the problem is 10x larger while the step
...
infeasible for any vehicle more than ~11.5 deg off vertical or rotating faster than
...
where a descent lives. Measured in flight: `Pa*Ae` approx.  334 kN, ~270 kN missing at the
~30% throttle flown, 2.2 m/s^2 on a 122 t vehicle.
...
*(Measured deflections were 0.1-2.2 deg, so this is worth ~0.1% - fixed because it is
...
gets multiplied by `Ixx ~ 1e8 kg m^2`. The demand was orders of magnitude past the
vehicle's authority and the loop went bang-bang. Parameterise by the closed-loop
frequency you actually want (`kAtt = wn^2`, `kRate = 2*zeta*wn`); a booster attitude
...
only from off-axis vernier gimbals, measured ~700x weaker than pitch/yaw). A vehicle
that happens to be yawed 180 deg is then required to roll 180 deg on its weakest axis for
...
One ADMM iteration costs ~2.8x more at N=80 than at N=30, so a fixed budget is a
...
(`build/native_src/blas_shim.c`), because `struct ACCEL_WORK` is private to that
...
that is the seed that matters - measured 5-90x cheaper than a cold solve at the gate.
...
(5 m/s^2), and add it to the planner's gravity. Estimating the *residual* rather than
...
`rho*||(X - Xbar)/Xscale||^2` - which restores exactly that conditioning *without* the
...
optimum genuinely has thrust direction jumping between nodes (19.1 deg node-to-node at
`W_DU = 0`, 3.9 deg at 0.05).
...
Two rules the logger must obey: **nothing may throw** (see section 8) and **nothing may
...
- **The plan itself spirals.** From a logged engage state the flight solver returned
  `IterationLimit` at sigma 27.2 s and swept **+319 deg** of bearing around the target,
  while the vehicle tracked it to 4 deg of thrust direction. The same initial condition
