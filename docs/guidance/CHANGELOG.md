...
- Added the **Mission planner** window (**AFC Guidance > Mission planner**). For now it plans a launch to meet the home body's moon: pick an arrival time and either the parking orbit's inclination or its LAN, and the other follows, so the plane contains the moon where it will be at arrival for an idealised Hohmann transfer injected opposite it. It shows each plane's next launch window, **LAUNCH NOW** sets the plane for a launch straight away, and **SEND TO ASCENT** makes the plane and parking orbit the Ascent tab's target, which EXECUTE then arms for.
