...
| `AdvancedFlightComputer/Features/Guidance/Conic` | Game-independent conic programs and the Clarabel and SCS bindings every solver uses. |
| `AdvancedFlightComputer/Features/Guidance/Gfold` | Game-independent G-FOLD solver. |
| `AdvancedFlightComputer/Features/Guidance/Scvx` | Game-independent successive-convexification solvers: the six degree of freedom landing in `SixDof`, the convex ascent in `Ascent`. |
...
The root solution builds the AFC mod, the separate harness test mod, four numerical libraries, and two console check projects.
The four numerical projects have no game references.
Only `Conic` calls native code, so both native solvers are copied beside it.
...
It includes the AFC assembly, the four numerical assemblies, both native solvers, XML patches, the root license, third-party notices, and the license texts in `licenses/`.
