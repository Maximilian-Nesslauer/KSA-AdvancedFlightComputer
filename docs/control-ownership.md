...
A step that throws keeps the craft on its last attitude target and engine command, because an engine cut in the air costs more than one bad step.
A failed step adds two to a count and a good step takes one away, so a steady fault releases the craft through `GuidanceWindow.FailAutopilot` after `GuidanceWindow.MaxFailedSteps` steps, and faults on more than a third of the steps release it in the end.
A craft with an engine cut queued, or a held craft on the ground outside an ascent, is released at the first failed step. An ascent keeps the window at lift-off. Each kind of fault is reported once.
...
| `FlightComputer.BurnPlan`, `Burn`, and `BurnMode` | `GuidanceWindow.StepStockDeorbit` and `ReleaseStockDeorbit` | Add a verified node, let stock Auto fly it, and stop Auto on an owned abort. Remove only the unchanged owned node; preserve a different loaded target. |
| `Vehicle._manualControlInputs.EngineOn` and `EngineThrottle` | `GuidanceWindow.StepStockDeorbit`, `ReleaseStockDeorbit`, and `ApplyAutopilot` | Preview timing with the engine off at minimum throttle, then restore an unchanged throttle setting. Hold the engine off while planning braking from the flown orbit. |
| `Universe` time warp | `GuidanceWindow.StepStockDeorbitWarp` and `ReleaseStockDeorbit` | Warp on request and stop before stock alignment. Cleanup stops only the warp owned by this request. |
| Sequence activation | `GuidanceWindow.AutoSequence` through `StagingDetector.Arm` and `StagingDetector.RequestStaging` | Guidance arms the AutoStage feature for the craft it flies and asks it for a row on a cold ignition or at the reserve boundary; the feature's own executor activates the row, fires held modules after their delays and never refreshes the part tree itself. A request made while a staging is still in flight is refused, and the reserve's hand-over is recorded only for a request the feature kept, so the reserve cue asks again after the cooldown. Guidance arms the feature once per flight, and after that the switch is the player's, so a disarm holds for the rest of the flight and a re-arm is used as it is. The hand-back disarms only what guidance armed and the player left on. Staging is not a reversible setting restored on release. |

`afc-guidance-driver` installs these hooks and steps the universe with the ascent step replaced. It checks the claim, the attitude and engine writes, the rate on the worker, the burn-mode hold with its takeover, replaced-burn and no-cut cases, the sink dispatch, the Enabled switch, a readout fault that leaves the mode flying, a run of failed steps that releases the craft, faults on every other step that release it later, a rare fault that does not, and a queued cut that releases it at once. `afc-guidance-staging-arm` checks the arm-once rule against the real AutoStage switch.
