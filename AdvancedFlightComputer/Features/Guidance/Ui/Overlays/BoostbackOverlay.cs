...
public static partial class GuidanceWindow
{
    ...
    /// <summary>Path samples Current. At the default stride that covers a several-minute
    ...
    /// overlay because the Jacobian is three seeded sweeps - about four times the cost of the prediction itself - and someone who only wants to see where the vehicle
    ...
    /// slower than the prediction: it costs about 3.6 ms, and the direction it produces moves far more slowly than the impact point does. Guidance passes its own,
    ...
    /// A staging event or a burn moves the impact point kilometres, and sliding a marker across a continent over a tenth of a second looks worse - and reads as
                                               ...
                                               bool force)
    {
        ...
        {
            Mu = parent.Mu,
            // The body spins about CCI +z (IParentBody.GetAngularVelocityCci is (0,0,w) and the game relies on it), which is exactly the frame DragCoastSystem is written in, so this is a scalar and not a vector.
        ...
        // FIRST PASS, deliberately coarse and with no path recorded. Its only job is to find out roughly WHERE the vehicle lands so the terrain height there can be sampled; a kilometre of error in that lookup point costs a few metres of terrain height, which the second pass then absorbs. Running it at the accurate step sizes would double the cost of the whole overlay to refine a number that is about to be low-passed anyway.
        ...
        if (p.Hit && parent is Celestial celestial)
        {
            ...
            {
                // Low-passed, not used raw. The sample point moves every recompute, and over rough ground the height under it can change by hundreds of metres - which a shallow descent turns into kilometres of lateral impact movement. See the header.
        ...
        // Convert the whole path to body-fixed coordinates ONCE, here. Each sample carries its own time, so each is carried back by the rotation that will have happened by the moment the vehicle reaches it.
        ...
        // Downrange over the GROUND, from the point below the vehicle now to the point it will hit - both body-fixed, each at its own time.
        //  Not the inertial angle between the two position vectors, which is the easy thing to write and is a different number: the body turns under the coast, so on a 200 s flight the two differ by most of a degree, which is around 90 km at the equator. The ground answer is the one that agrees with the marker this is drawn beside.
    ...
    ///  Runs three seeded sweeps for d(ground impact)/d(v), then solves the damped, tangent-projected least squares in ImpactSteering. Throttled hard and gated on its own toggle - this is the expensive thing on this overlay.
    ///  The interval is a parameter because there are two callers with different needs:
    /// the arrow, which only has to look right, and the boostback burn, which FLIES the answer and wants it fresh (see BoostbackSteerIntervalMs). Both share the one throttle and the one result, so having both on screen costs nothing extra.
    ///  Returning early leaves HasSteer and SteerDv alone rather than clearing them, which is what lets the guidance step hold the last solution between solves.
    /// </summary>
                                       ...
                                       long intervalMs = SteerIntervalMs)
    {
        ...
        //  The prediction that DRAWS the impact marker terminates on the terrain under the impact point, which is right for drawing: that is where the vehicle would actually touch. It is wrong for TARGETING, twice over. It puts the hit and the target on two different spheres, so the miss carries a radial component that is a difference of terrain heights rather than a distance to fly; and that height MOVES as the impact point walks across the terrain, so the surface the solution is converging onto wanders under it - a low-passed wander, at that, which is worse than a noisy one because it lags.
        //  Aiming both at the site's own radius makes the miss purely tangential and makes it mean what it says: how far along the ground the impact is from the pad. The site's terrain is cached per lat/lon, so it is also a constant, and the burn converges onto a surface that is standing still.
        //  It matters more than it looks. At a steep impact the downrange error from a wrong landing radius is dh/tan(gamma) - a couple of hundred metres of terrain is over a hundred metres of miss, which is the same size as the miss itself.
        ...
        // The miss, in the frame J's ROWS are written in - the co-rotating frame, which is the current body-fixed frame turned back into CCI axes. The site is body-fixed, so it converts with Ccf2Cci; getting this backwards would give a correction that is right in magnitude and wrong by the body's rotation.
    ...
    ///  THE PROBLEM. The targeting correction minimises dv and nothing else, and the cheapest way to drag an impact point backwards is often to thrust DOWNWARD - a steeper flight path shortens the time of flight and pulls the impact in. On the reference arc the correction has 94.5 m/s of its 173.7 pointing below the local horizon. That is the booster visibly diving during its own boostback, and it is the documented failure of instantaneous impact-point guidance (Jo, Han and Ahn section 2.4: "if the command associated with the maximum IIP rate falls within this region, the IIP guidance algorithm generates a descending trajectory").
    ///  WHAT IS TARGETED, AND WHAT IS NOT. The COMMANDED DIRECTION is levelled - the burn is stopped from pointing down. The vehicle's own flight path angle is left alone. That distinction is worth being exact about because getting it wrong is expensive: this booster is past apogee and descending at 13 degrees, and levelling its VELOCITY costs 441.9 m/s against a 173.7 m/s correction. Cancelling the descent is not the boostback's job. Not thrusting further into it is.
    ///  WHY IT IS NEARLY FREE. J maps three velocity components onto two impact directions, so one combination of velocity moves the impact point nowhere at all. Pushing along that direction re-aims the burn without disturbing where the vehicle lands - the standard task-priority arrangement from redundant-manipulator control, primary task untouched by construction. It is free only to FIRST order:
    /// a finite push leaks a little, measured at about 6% of what the same dv would move the impact if spent on steering.
    ///  A FLOOR, NOT A SETPOINT, and the caps are what make it one. Fully levelling the command costs 119 m/s here, most of another correction; the caps hold the spend to something proportionate and leave the burn partly nose-down rather than pretending it can be free. The paper's own scheme is likewise "activated when the flight-path-angle rate falls below the predefined value", with that value
    ...
    private static void ShapeFlightPathAngle(double3 r0, double3 v0, ReadOnlySpan<double> dG)
    {
        ...
        // A FLOOR. Only ever raises the command; a burn already pointing high enough is left alone, because forcing it back DOWN would be the same mistake in the other direction.
        ...
        // Point the free direction UP, so more of it always means more pitch. Both signs are equally free; only the sense differs.
        if (vertical < 0.0)
        {
            n = -n;
            vertical = -vertical;
        ...
        // THE CEILING, and it is geometry rather than policy. cmd(t) = dv + t*n, so as t grows the direction tends to n itself and the pitch tends to n's pitch.
        ...
        {
            // Refused rather than approached: the last couple of degrees before the asymptote cost more dv than the whole rest of the range put together.
        ...
        // Solve pitch(t) = target. The map is monotonic in t (verified by --impact), so bracket by doubling and bisect - bulletproof, and cheap because each evaluation is one vector add and a normalise. Newton would be faster and would stall near the asymptote, which is exactly where robustness matters.
    ...
    ///  The de-rotation by omega * (time of flight) is NOT done here - the predictor has already done it, in Dual arithmetic, and reports the result as Fx/Fy/Fz.
    /// This only applies the current CCI->CCF orientation, which is a constant as far as the trajectory is concerned.
    ///  Doing the de-rotation here instead would work for the display and be wrong for anything differentiated: the rotation angle contains the time of flight, so rotating the .V parts outside the Dual chain drops the d(tof)/dv term, which is worth about half of d(ground)/dv (measured by --impact). Reading the predictor's own answer is what stops the picture and the sensitivities disagreeing.
    /// </summary>
    ...
    private static void DrawBoostbackOverlay(IGameViewport vp, Vehicle vehicle, Orbit orbit,
                                             IParentBody parent)
    {
        // The FOCUSED vehicle's prediction, resolved here rather than read from the ambient current - the same trap the 6-DOF overlay documents.
        ...
        // Blend the DRAWN point toward the latest prediction. The prediction itself steps at the recompute rate; without this the marker teleports sixteen times a second by however far the answer moved, which reads as noise even when every step is a correct answer. Large moves snap - see ImpactSnapDistanceM.
        long now = Environment.TickCount64;
        ...
        {
            // A ringed crosshair rather than a bare cross: over terrain at any zoom the ring is what makes it findable, and the gap in the middle leaves the actual point visible instead of covering it.
            ...
            // Shadowed text: the overlay has no depth test and draws over terrain, ocean and cloud, and plain red on a bright desert is unreadable.
        ...
        // A line from the landing site to the predicted impact: the miss distance is what a boostback is trying to drive to zero, and seeing it is worth more than reading it. Body-fixed at both ends, so it does not swing about.
        ...
        if (_showSteerArrow)
        {
            ...
            if (st.HasSteer)
            {
                ...
                // The greedy direction, thinner and dimmer. Drawn because the two are easy to assume identical and are not: -J^T m is a descent direction, -J^+ m is the one that points at the target.
