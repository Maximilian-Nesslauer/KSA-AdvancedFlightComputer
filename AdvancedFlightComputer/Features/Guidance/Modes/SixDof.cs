...
public static partial class GuidanceWindow
{
    ...
    // 50. Measured in closed loop (Scvx.Console --mpc, zero dispersion, so any plan movement is the model's own error rather than disturbance): nodes miss path/direct plan jump ADMM/cycle 20 1.9 m 1.50 4.0 m 2377 30 2.1 m 1.36 2.4 m 1785 50 1.6 m 1.28 1.3 m 1988 80 4.3 m 1.25 1.0 m 821
    //
    // The plan jumping between re-solves is discretisation error - the gap between the trapezoidal collocation and the true dynamics - and it falls monotonically with node count.
    // No weight, scale or conditioning change moved it at all.
    // The over-long curved path improves with nodes too, and the vehicle never actually moves AWAY from the target (away-from-target is 0.0 m at every node count), so the "loops" are a long curve, not a loop.
    //
    // 20 was the worst point on this curve on all three reported symptoms at once - most plan jump, longest path, and the MOST ADMM iterations.
    // More nodes does not cost more here: the smoother problem converges in fewer ADMM iterations.
    /// <summary>
    ...
    //
    // The cold count is now derived from the same spacing target as everything else, so there is one criterion rather than two, and it cannot drift away from what the ladder believes.
    // Cheapness is not a property worth having on its own: a plan the vehicle cannot follow costs a cold restart, and the log shows three of them in ten seconds before it gave up entirely.
    ...
    // Tilt cone, in degrees from vertical.
    // 120 degrees suits a booster entry better than 60 degrees.
    // This vehicle arrives in a 92 degree belly-flop.
    // It must rotate upright under thrust.
    // A cap below the entry attitude would be infeasible.
    // The cone applies at node 0.
    // Node 0 is pinned to the measured state by equality.
    // Approach corridor and climb limit.
    // Both constraints are soft.
    // They start at node 1 and use penalised slack.
    // This makes the aggressive defaults safe.
    // Ten degrees above horizontal prevents a wide path without forcing a narrow corridor.
    // Zero means "never climb".
    // A climbing vehicle then gets an expensive plan instead of no plan.

    // Terminal hover takes the last stretch.
    // It is on by default and hands over above the target altitude.
    // This avoids the part of the trajectory the solver handles worst.
    // Cadence is in wall-clock seconds.
    // The scale-free quantity is plan nodes.
    // Node spacing is sigma/(N-1).
    // As sigma shrinks, a fixed interval covers more nodes.
    // Seconds match the frame budget and are easier to use while flying.
    //
    ...
    // Past about 2 nodes, the warm start is too stale and the tight trust region fails.
    // At the default 0.1 s and N=80, a 20 s burn has 0.25 s per node.
    // The interval then covers 0.4 nodes.
    // The bound tightens as sigma falls.
    // The readout also reports cadence in nodes.

    // Objective regulariser weights.
    // Both were originally the Python test case's (W_DU 0.2, W_W 1.0), where they came to 121% of the fuel term - dominating the objective and, because both shrink as burn time grows, pinning sigma at its upper bound.
    // Turned down twice after flight testing.
    // Exposed because the right value depends on the vehicle and is easiest to find by flying it.
    // W_DU.
    // Raised back to 0.05 after flight testing: unlike W_W (which was pinning burn time at its upper bound and deserved cutting), THIS term has a distinct job - it is the only thing keeping the control profile CONTINUOUS.
    // Min-fuel with no control-rate penalty is bang-bang, and the optimum genuinely has the thrust direction jumping between nodes.
    // Measured on a realistic start:
    //   W_DU 0.000 -> 19.1 deg thrust-direction jump node-to-node (visible kinks)
    //   W_DU 0.010 ->  5.9 deg
    //   W_DU 0.050 ->  3.9 deg
    // It also suppresses out-of-plane wandering (15.9 m -> 8.3 m) by discouraging the solver from hopping between local optima.
    // Cost is a slightly longer path.

    // Proximal conditioning.
    // The regularisers above were also holding P together; turning them down for a correct trajectory tripled the ADMM iteration count.
    // This puts the conditioning back without the bias. 0.05 keeps the answer and is ~3.8x faster; higher is faster still but starts moving the solution.

    // Fixed burn time (see Ksa6DofGuidance.FixedTime).
    // Free final time makes the dynamics bilinear in (sigma, x, u) and is the root of sigma pinning, the regularisers biasing the trajectory, and loitering.
    // Gfold already works this way.
    ...
    // Touchdown latch.
    // A vehicle sitting on the pad ALREADY reports terrain contact (the launch-pad collider counts), so "cut on contact" must not fire until the vehicle has first been observed OFF the ground.
    ...
    /// </summary>
    // NODE SPACING TARGET, seconds.
    // Measured on this vehicle, flight defect against the spacing that produced it:
    //
    ...
    //                              dt 0.46 s -> 0.52 m
    //
    // The knee is sharp and it sits between 0.79 s and 0.65 s: above it the plan is at or over the 1 m gate and the loop refuses everything it produces, below it there is an order of magnitude of margin. 0.85 s was on the wrong side of that knee, which is why the ladder kept asking for counts whose plans could not be flown.
    ...
    // Consecutive refused re-solves since the last node step, and the coarsest rung index the ladder may still move to.
    // A refused plan means the vehicle is flying the previous one OPEN LOOP, so a step that causes them has to be undone - one-way stepping on its own cannot recover from a rung that does not work.

    // Speed at which the rung floor was set.
    // A refusal is evidence about the conditions it happened in, NOT about the whole descent, and this is what lets the floor expire once those conditions have gone.
    // Rung index a back-off has already been attempted from, so a failed attempt is not repeated. -1 means none tried yet.

    // Refusals tolerated before undoing a node step.
    // Long enough that one awkward cycle does not trigger it, short enough that the vehicle is not open loop for long: at a 0.1 s cadence this is half a second.
    private const int RefusalsBeforeBackOff = 5;

    // Refusals after which the plan is abandoned and cold-started again.
    // Well past the back-off threshold, so the cheaper remedy is tried first, but far short of the ~100 consecutive refusals seen in flight log 20260807-093052.
    private const int RefusalsBeforeRestart = 15;

    // Drift past which the warm seed has stopped being worth having and a cold restart is the cheaper option.
    // Taken as the larger of this and 5% of altitude, so it scales with how much room there is: 20 m matters at 100 m and is noise at 2 km.
    private const double ColdRestartDriftM = 20.0;

    // Seed the cold solve from a convex 3-DOF G-FOLD solve.
    //
    // OFF, because it was MEASURED not to help (Scvx.Console --seed).
    // The idea is sound in general - SCvx refines a reference rather than searching for one - but there is almost no headroom to win here: at the node counts now flown the straight-line seed already converges in 6 to 8 SCvx iterations and 130-260 ms total, so a seed costing 300-1000 ms to compute cannot pay for itself, and on several cases it made the first subproblem fail outright.
    //
    // Kept behind a flag with its test, because it should be revisited if cold-solve cost ever becomes the bottleneck again - at higher node counts, or from a much worse initial state, the balance would be different.

    // Spread the cold solve over frames instead of blocking on it.
    //
    // A blocking cold solve costs 130-280 ms in the sim thread's frame, which is a visible hitch.
    // SCvx is iterative and the solver keeps its state between calls, so the same work can be done one iteration per frame.
    // Measured, a flyable plan arrives in about three frames and the vehicle falls 1-6 m in the meantime - absorbed automatically, because each frame re-anchors at the measured state exactly as the warm loop does.
    // Gap between cold-solve iterations, in seconds.
    // At 0.25 s the handful of iterations a cold solve needs lands across about a second, as separate short hitches rather than one merged freeze.
    ...
    {
        // RECOVERY.
        // A run of refusals means the vehicle is flying an ageing open-loop plan, and the commonest cause is a node step that landed on a count too coarse for the conditions - the flight log for 20260806-142450 shows a step to 5 nodes at 261 m taking the defect from 0.49 m to 390 m and refusing every re-solve thereafter.
        // One-way stepping cannot recover from that on its own, so step BACK UP one rung and refuse to go that coarse again for the rest of the run.
        // GateIndex > 0 IS A SEPARATE CONDITION FROM GateIndex > BackedOffTo, and both are needed.
        // The second asks "have we already backed off from this rung this run", and BackedOffTo starts at -1 meaning never - so at the FINEST rung (index 0) it reads 0 > -1, passes, and lets finer become -1.
        // NodeRungs is ordered finest-first, so index 0 is 50 nodes and there is nothing finer to step to; NodeRungs[-1] then throws while evaluating the argument to RebuildAt.
        //
        // GateIndex reaches 0 on the ordinary path - the ladder asking for 50 while the guidance already runs at 40 sets it there to record "already finer than asked" - after which the vehicle sits one refusal run away from the crash for the rest of the flight.
        // Four flights hit it (6dof-faults.log, gate=0 rungFloor=-1 every time); 20260821-093905 died at 360 m on five consecutive refusals of 1.1-1.2x tolerance, RefusalsBeforeBackOff being exactly 5.
        //
        // The guard also protects RungFloor below: with finer = -1 it was set to -1, which as an index floor means no rung is permitted at all, and would have silently disabled the node ladder for the rest of the run had the throw ever been swallowed rather than ending the flight.
        if (_s.RefusalRun >= RefusalsBeforeBackOff && _s.GateIndex > 0
            && _s.GateIndex > _s.BackedOffTo)
        {
            int finer = _s.GateIndex - 1;
            // Record the attempt BEFORE trying, so a rebuild that fails is not retried every five cycles forever.
            // Flight log 20260807-093052 did exactly that: the same back-off to 25 nodes failed and was retried ~40 times, each one a full config build plus a failed cold solve, which is pure added cost on top of an already-failing loop.
        ...
        // Nodes needed to hold the target spacing at the CURRENT burn time.
        // Sigma is what sets spacing, and sigma counts down through the descent, so this falls naturally without reference to altitude at all - and it adapts to a fast entry or a slow one, which an altitude ladder cannot.
        double sigma = _s.Guidance.Sigma;
        if (sigma <= 0.0)
            return;
        int ideal = (int)Math.Ceiling(sigma / Math.Max(_s.SixDofNodeDtTarget, 0.05)) + 1;

        // Snap to a rung: the COARSEST count that still delivers at least the resolution asked for.
        // Falls back to the FINEST rung, not the coarsest, when the burn is long enough that no rung reaches the target - wanting more resolution than is on offer must give the most available, and defaulting the other way would hand a 60-node problem 5 nodes.
        int nodes = MaxNodes;
        for (int i = NodeRungs.Length - 1; i >= 0; i--)
            if (NodeRungs[i] >= ideal) { nodes = NodeRungs[i]; break; }
        nodes = Math.Clamp(nodes, MinNodes, MaxNodes);

        // One-way only: never step back up.
        // Sigma wobbles cycle to cycle, and rebuilding the solver to add a node the problem does not really need costs a warm start for nothing.
        int want0 = Array.IndexOf(NodeRungs, nodes);
        if (want0 < 0)
            return;
        // EXPIRE THE FLOOR WHEN THE CONDITIONS THAT SET IT HAVE GONE.
        //
        // A refused rung is evidence about the state it was refused in, not a fact about the descent.
        // Collocation defect follows how fast the state is changing, so a rung that could not hold 130 m/s at 92 degrees off vertical says nothing about the same rung at 34 m/s upright - and treating it as permanent pins the whole approach at a count it stopped needing.
        //
        // That is exactly what flight log 20260807-103232 did: one refusal at 1738 m fixed the floor at 25 nodes, and the run then flew all 18 s to touchdown at 25 while the ladder was asking for 20 and then 10, with defect margins of 24x to 92x on the gate and solves costing 75-330 ms that should have cost ~16 ms.
        // The user read that as being too coarse near the end; it was the opposite, and this is why.
        //
        // Speed rather than altitude, because speed is what the defect tracks.
        // A third off is a material change and cheap to re-test: the retry costs one refused reseed, and if conditions really have not eased it just sets the floor again at the new speed.
        ...
        // Never move coarser than a rung already shown to be unflyable in CURRENT conditions - see the expiry above.
        ...
        // The step was refused, which means this rung is too coarse to produce a flyable plan for the trajectory actually being flown.
        // Remember that: the node-spacing target is a PROXY for collocation error, and how good a proxy it is depends on the manoeuvre.
        // Measured offline on gentle descents, 0.5-1 s spacing costs 0.02 m of defect; on the logged 81 m/s, 92-degrees-off-vertical entry the same spacing cost 1.13 m, twenty times more, because defect follows how fast the state is changing and not spacing alone.
        // Rather than retune a number that cannot be right for every case, let the rung floor record what this descent will actually take, and stop asking again.
    ...
    private static void OnSolveSucceeded(double now, double[] x)
    {
        ...
        _s.SolveOk = true;
        _s.RefusalRun = 0;
        // Worth an event, not just a column: this fires exactly once per real sign flip - the plan is stored on the vehicle's branch afterwards, so the next cycle finds nothing to do - and before the fix each one cost fifteen refusals and a cold restart.
    ...
    // A/B TOGGLE.
    // The synchronous path is kept in full rather than removed, because every real bug this feature has produced was found in a flight log, and being able to switch back mid-descent is worth more than the duplicated branch costs.
    ...
    // Set by the cadence gate so a row can say whether THIS cycle re-solved, rather than only reporting the last solve's result forever.
    // Without that distinction a run of refusals is indistinguishable from a run of cycles that simply were not due to solve.
                                 ...
                                 double throttle, double3 torqueModel, double ambientPa)
    {
        ...
        // Signed distance outside the glideslope cone, same convention the solver enforces: positive means outside.
        ...
        var row = new SixDofLog.CycleRow
        {
            ...
            // MeasureDrift ran earlier in this same step (see the cold-restart branch), so this is the value that decision was actually taken on rather than a second, slightly different measurement of the same thing.
    ...
    private static void UpdateAccelBias(double[] x, double now, double thrustDelivered)
    {
        ...
        // Modelled acceleration: thrust along the body +Z axis, plus the gravity the config was built with (NOT the biased value, or the estimate feeds itself).
        ...
        // ~2 s time constant at a 0.02 s step: slow enough to ignore per-frame noise, fast enough to follow drag falling away through the descent.
        ...
        // PUBLISHED, not applied.
        // The guidance folds it into the model at the entry to its next solve and not before, so a solve already running keeps the model it started with.
        // See Ksa6DofInputs.
    ...
    private static void TerminalAttitude(double[] x, double[] xf)
    {
        ...
        if (m < 1e-9)
        {
            // Tilted a full 180 degrees, so there is no yaw to preserve.
            // Identity is as good an answer as any and keeps the quaternion a unit.
    ...
    private static void Disengage6Dof(Vehicle vehicle, bool cutEngine = true)
    {
        ...
        // Disengage is also reached from the ImGui draw, so nothing here may block or throw - see Ksa6DofSolveWorker.Dispose.
    ...
    //
    // Draw-time writes to the flight computer or _manualControlInputs are erased one frame later, so the tab only sets flags and every command is issued here.
    internal static void Step6Dof(Vehicle vehicle)
    {
        Use(vehicle);

        // Caught here rather than left to Mod's prefix handler, which logs and swallows - for an engage failure that is indistinguishable from the button doing nothing at all.
        try
        {
        ...
        catch (Exception e)
        {
            ...
            // "Index was outside the bounds of the array" names neither the array nor the line, and this catch then disengages - so by the time anyone looks, the state that produced it is gone.
            // The full exception goes to a file that always exists, with enough surrounding state to tell which of the several paths through a step was running when it threw.
    ...
    private static void Step6DofCore(Vehicle vehicle)
    {
        ...
        // The old guard existed only because one set of statics had to serve every craft.
        // STAGING TERMINATES GUIDANCE ON THE STAGE THAT LOST PARTS.
        //
        // KSA keeps the SAME Vehicle object through a separation - it calls UpdateAfterPartTreeModification on it - and the discarded stage becomes a new one.
        // So keying state on the vehicle already does most of the work: the upper stage carries its guidance across the event, and the spent booster arrives as a craft with no state and is simply not flown.
        //
        // What that does NOT cover is the vehicle that KEPT the state suddenly being a different rocket.
        // Tmax, the inertia and the whole trajectory were built for a stack that no longer exists, and the plan is not merely stale but wrong about the vehicle.
        // Terminating is the honest default: re-engaging is one button, and it rebuilds the config against what is actually there.
        ...
        // Same trace the descent plot draws for G-FOLD and hover, so the 6-DOF page shows a flown path against its plan too.
        if (_s.Active)
        {
        ...
        // Keep the worker until its current result is collected, so inline work cannot share the same mutable solver.
        ...
        // CIRCUIT BREAKER.
        // A long run of refusals means the plan is stale, the vehicle has diverged from it, and every further Update is running the wide-trust- region retry - so failure costs MORE than success and the loop digs itself deeper.
        // Once that happens the answer is not to keep hammering it: throw the plan away and cold-start again, spread over frames so it does not stall the sim thread.
        // ESCALATION LADDER.
        // Each rung is only reached when the cheaper one below has stopped working.
        //
        ...
        //
        // The two triggers need different remedies because they identify different faults:
        //
        // - Drift means the vehicle is not where its plan says, so rung 4 replaces the inconsistent trajectory with a straight-line seed.
        //
        // - A run of refusals means the plan failed the defect gate, even when the geometry is usually fine and drift is nil.
        // - The loop cannot take an accepted step, so a straight-line seed would discard a converged trajectory for one that is not dynamically feasible.
        // - Rung 3 keeps the plan and rebuilds the solver around it.
        //
        // When the loop is stuck, change the solve rather than the trajectory.
        // Reads the PUBLISHED plan, so it is safe while a solve is in flight - see Ksa6DofGuidance.MeasureDrift.
        ...
        // GuidanceIdle: a cold restart calls BeginCold, which reinitialises the solver out from under anything using it.
        // While a solve is in flight the worker owns the guidance outright, so this waits - a cycle of delay on a restart that only fires after fifteen refusals is not a cost worth worrying about.
        ...
        if (_s.Converging)
        {
            ...
            // THE COLD SOLVE IS A SOLVE TOO.
            // Threading Update alone left this running inline, and flight log 20260809-113132 hitched throughout because of it - three cold solves of a dozen iterations each, every one on the sim thread at up to 300 ms a piece.
            //
            // Dispatched one iteration at a time rather than looping on the worker, so the anchor stays as fresh as it was inline: at 126 m/s the vehicle falls over a hundred metres during a cold solve, and freezing x0 for the whole of it would seed the first warm cycle from where the vehicle used to be.
            bool coldDone;
            string coldError;
            if (coldResult != null)
            {
        ...
        // TOUCHDOWN.
        // Without this the guidance kept commanding the plan's terminal control forever after arriving - nothing watched the ground, nothing cut the engine.
        // Armed only once the vehicle has been seen OFF the ground, because a vehicle on the pad already reports contact against the pad.
        //
        // This MUST live in the step, not the draw: the draw only runs when the tab is open, and writes from it are erased by the sim copy-back a frame later.
        ...
        //
        // The final metres are the worst part of the trajectory for this solver and the easiest for the hover PID.
        // The horizon has collapsed to almost nothing, so the plan is a handful of nodes over a second or two and the trust region is the binding constraint; meanwhile the terminal state is exactly what a hover controller is built for - near-zero velocity, upright, holding a point.
        // Optimising a descent is the wrong question by then.
        //
        // Handing over ABOVE the target altitude on purpose: the 6-DOF plan aims at _s.SixDofTargetAltM and this fires on the way down to it.
        // A handover set below the target would simply never trigger, which the UI warns about.
        ...
        // Estimate what the model is missing BEFORE re-solving, so the optimiser plans around it rather than rediscovering it every cycle.
        UpdateAccelBias(x, now, _s.LastThrottle * _s.CapabilityN);

        // NODE SCHEDULE: drop node count at fixed altitude GATES.
        //
        // The quantity worth holding roughly constant is node SPACING, not node count.
        // Node spacing is sigma/(N-1), so as the horizon collapses on approach the same N buys ever finer resolution than the problem needs, and the solve is paying for nodes that are no longer earning anything.
        // The earlier measurement that fewer nodes means more plan jump (4.0 m at N=20 against 1.0 m at N=80) was taken over a FULL-LENGTH trajectory, where N=20 meant coarse spacing; close in, the same count is fine.
        //
        // Stepped at gates rather than recomputed continuously, on purpose.
        // Every change of N rebuilds the solver and throws away the ADMM warm start (the sparsity pattern is frozen at the node count), so a continuously-tracked N would pay that on EVERY cycle and never keep a warm start at all.
        // Four transitions over a descent is affordable; four hundred is not.
        //
        // One-way, too: the ladder never steps back up.
        // Re-crossing a gate during a brief climb would rebuild twice for no benefit.
        // Same rule: StepNodeGates can call RebuildAt, which constructs a new guidance and assigns _s.Guidance.
        // Doing that while the worker holds a reference to the old one would leave a solve running on an object nothing is going to read.
        if (_s.SixDofNodeGates && GuidanceIdle)
            StepNodeGates(vehicle, parent, siteCci, x, now);

        // THE MPC STEP: re-solve from the MEASURED state on a cadence.
        // This is where all the feedback in the system comes from - there is nothing else.
        //
        // The cadence timer advances ONLY ON SUCCESS.
        // Previously it was stamped before the attempt, so a failed re-solve burned the whole interval before trying again while Command kept advancing the plan clock - the plan's time index ran on along a trajectory that was never refreshed, which is the "green dot outruns the vehicle" symptom.
        // A failed solve now retries on the next step instead of letting the clock run.
        ...
        // Past the end of the plan the node index clamps, so the terminal control is held indefinitely.
        // That is a reasonable hold-in-place fallback, but it is NOT guidance any more and must not look like it.
        ...
        //
        // This is what makes the commanded thrust the delivered thrust regardless of engine performance.
        // The plan's Tmax is fixed at solve time and evaluated at the TARGET altitude, so it is deliberately conservative and drifts further from the truth the higher the vehicle is; dividing by it under-throttles by exactly that ratio the whole way down.
        // MPC cannot see this - re-solving fixes the state estimate, not the actuator mapping, so every replan is executed just as wrongly as the one before.
        // That is the shortfall that made the vehicle sink below its own plan until only a loop was feasible.
        ...
        // Thrust is not proportional to throttle in an atmosphere - see KsaEnginePerf.ThrustAtThrottle - so demand/capability under-delivers by a near-constant amount, worst at low throttle, which is precisely where a descent spends its time.
        //
        // Falling back to the linear estimate when there is nothing to invert is not a nicety: before ignition capability is 0 and dividing gives NaN.
        ...
        // Model body axes -> KSA body axes.
        // The allocator works in KSA's frame; a missed conversion here would put roll torque on the pitch axis.
        ...
        // The override used to be a single global target, so a second guided craft silently stole the first one's gimbals.
        KsaGimbalControl.SetLsq(vehicle, torqueKsa);

        ref ManualControlInputs manual = ref ManualInputs(vehicle);
        // ENGINE STAYS LIT WHILE GUIDING.
        // This was `throttle > 0.02`, copied from the G-FOLD path where it is correct - that planner has NO thrust floor and plans genuine coasts, so a near-zero command means "actually stop burning".
        //
        // The 6-DOF model is the opposite: its throttle box guarantees T >= Tmin > 0, so it NEVER asks for a coast.
        // And KSA clamps a vehicle's MinimumThrottle as low as 0.01, so with the floor now read from the vehicle a perfectly normal min-fuel opening command of 1% fell below the 2% threshold and shut the engine down.
        // That loses THRUST AND TORQUE together - gimbals have no authority without thrust - so the vehicle went into free fall with no attitude control on the very first step.
                                         ...
                                         out Scvx6DofConfig cfg, out Dynamics6Dof.Params dyn, out string error)
    {
        ...
        //
        // It used to be computed by the 6-DOF TAB'S DRAW, which worked only because the Engage button lived on that tab - you could not engage without having drawn it.
        // The gauge panel engages from anywhere, so a craft that never had that tab opened planned against 0.40 and refused to solve from states it handles easily.
        // Model values must not depend on which page is on screen.
                                   ...
                                   double[] x, double now)
    {
        ...
        // SEED FROM G-FOLD.
        // SCvx refines a reference rather than searching for one, so the seed decides how many iterations the cold solve needs and which local solution it walks toward.
        // G-FOLD solves the same landing under a convex 3-DOF model in a few milliseconds, with no initial guess of its own and no local minima, and its golden-section search over time of flight also supplies a burn time far better than a fixed guess.
        //
        // Strictly an optimisation: any failure falls back to the straight-line seed, because a worse guess is enormously better than not engaging.
        // SPREAD: start the cold solve and return.
        // Step6DofCore advances it a bounded slice at a time until the plan is flyable, so engaging costs a few milliseconds of this frame rather than the whole solve.
    ...
    // Whether a plan can exist AT ALL, shown before engaging rather than after a failed solve.
    // Thrust-to-weight at MINIMUM throttle is the number that decides it: the model's throttle box means the engine cannot go below Tmin while lit, so if Tmin exceeds weight the vehicle must tilt to shed the excess, and beyond the tilt limit no descent exists.
    // That single ratio explains infeasible solves, spiral trajectories, and the long solve times that come from retrying them.
    private static void Draw6DofFeasibility(Vehicle vehicle)
    {
        ...
        // Must use the SAME pressure-corrected thrust the planner does.
        // This panel reading vacuum thrust while the plan used something else is precisely how the atmospheric shortfall stayed hidden: "max TWR" looked healthy because it was quoting a number the vehicle could only reach in space.
        double ambientPa = KsaEnginePerf.AmbientPressureAt(
        ...
        // On an airless world this reads 0 Pa / 100% and the whole line is a no-op; anywhere else it is the number that decides whether the plan is flyable.
        ...
        // The question the rest of the panel does not ask.
        //
        // Thrust only buys deceleration ABOVE hover, so the usable figure is (TWR - 1) * g, not TWR * g.
        // Stopping from speed v therefore needs v^2 / (2 * (TWR-1) * g) of altitude, and if that exceeds what is left the landing is impossible for ANY guidance law - the solver will return its best effort, which is a long curving miss that looks exactly like a controller bug and is not one.
        //
        // Measured in closed loop against this criterion (Scvx.Console --mpc --grav, 300 m up at 50 m/s down): TWR 2.00 needs 127 m and misses by 4.8 m; TWR 1.50 needs 255 m and misses by 39 m; TWR 1.25 needs 510 m of the 300 m available and misses by 173 m.
        // The failure tracks reachability, not conditioning - Earth at TWR 2.0 actually solves FASTER than the Moon.
        //
        // A necessary condition, not a sufficient one: it assumes thrust straight up, so any tilt to kill downrange makes it worse.
